...
class ScenarioManagement:
    ...
    def start_physical_nodes(self):
        ...
        for idx, node in enumerate(self.config.participants):
            ...
            with open(f"{self.config_dir}/participant_{idx}.json", "r") as f:
        ...
        try:
            if self.host_platform == "windows":
                ...
                for node in sorted_participants:
                    ...
                    commands += f"OUT_FILE={self.root_path}/app/logs/{self.scenario_name}/participant_{node['device_args']['idx']}.out\n"
                    commands += f"python3 {self.root_path}/nebula/node.py {self.root_path}/app/config/{self.scenario_name}/participant_{node['device_args']['idx']}.json > $OUT_FILE 2>&1 &\n"
